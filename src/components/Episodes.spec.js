...
const mockEpisodes = [
  ...
  test('Renders list of epidodes with mock data', () => {
      const { queryAllByTestId, getAllByText} = render(<Episodes episodes={[]} />);
      const episode = queryAllByTestId(/episode/i)
      expect(episode).toHaveLength(0);

      render(<Episodes episodes={mockEpisodes} />)
      const episode2 =getAllByText(/episode/i);
      expect(episode2).toHaveLength(3)
  })